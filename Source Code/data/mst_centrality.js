function createNetworkVisualization(error, graph, centralityMeasures) {
  ...
  updateCentralityMeasure(centralityMeasures[1]);
...
//       .force("collide", d3.forceCollide(function(d) { return circleSize.max; }))
  ...
  function createCentralityControls() {
  	controls = centralityMeasures.filter(item => item !== 'name')
    ...
    var headers = table.append("thead")
      ...
      .enter()
        ...
        .on('click', function (d) {
			...
			rows.sort(function(a, b) { console.log(a); console.log(b); return b[d] < a[d]; });
			this.className = 'aes';		   
	   });
    ...
    rows.selectAll('td')
      .data(function(node) {
        var measures = centralityMeasures.map(function(centralityMeasure) {
        	if(centralityMeasure == 'name') {
        		return node.centrality[centralityMeasure];
        	} else {
